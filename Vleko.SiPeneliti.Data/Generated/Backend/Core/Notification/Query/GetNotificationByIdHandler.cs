...
namespace Vleko.SiPeneliti.Core.Notification.Query
{
    ...
    internal class GetNotificationByIdHandler : IRequestHandler<GetNotificationByIdRequest, ObjectResponse<NotificationResponse>>
    {
        ...
        public async Task<ObjectResponse<NotificationResponse>> Handle(GetNotificationByIdRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var item = await _context.Entity<Vleko.SiPeneliti.Data.Model.Notification>().Where(d => d.Id == request.Id).FirstOrDefaultAsync();
                if (item != null)
                {
                    ...
                    result.NotFound($"Id Vleko.SiPeneliti.Data.Model.Notification {request.Id} Tidak Ditemukan");
            }
            catch (Exception ex)
            {
