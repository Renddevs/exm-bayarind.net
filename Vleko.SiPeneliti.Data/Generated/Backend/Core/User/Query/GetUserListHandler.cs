...
namespace Vleko.SiPeneliti.Core.User.Query
{
    ...
    internal class GetUserListHandler : IRequestHandler<GetUserListRequest, ListResponse<UserResponse>>
    {
        ...
        public async Task<ListResponse<UserResponse>> Handle(GetUserListRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
				var query = _context.Entity<Vleko.SiPeneliti.Data.Model.User>().AsQueryable();

				#region Filter
				Expression<Func<Vleko.SiPeneliti.Data.Model.User, object>> column_sort = null;
				List<Expression<Func<Vleko.SiPeneliti.Data.Model.User, bool>>> where = new List<Expression<Func<Vleko.SiPeneliti.Data.Model.User, bool>>>();
        ...
        #region List Utility
		private (Expression<Func<Vleko.SiPeneliti.Data.Model.User, bool>> where, Expression<Func<Vleko.SiPeneliti.Data.Model.User, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<Vleko.SiPeneliti.Data.Model.User, object>> result_order = null;
			Expression<Func<Vleko.SiPeneliti.Data.Model.User, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
					...
					case "photobase64" : 
						if(is_where){
							result_where = (d=>d.PhotoBase64.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.PhotoBase64);
					break;
					case "token" : 
						if(is_where){
