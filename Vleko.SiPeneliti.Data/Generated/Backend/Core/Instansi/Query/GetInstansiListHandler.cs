...
{
    public class GetInstansiListRequest : ListRequest,IListRequest<GetInstansiListRequest>,IRequest<ListResponse<InstansiResponse>>
    {
    }
    internal class GetInstansiListHandler : IRequestHandler<GetInstansiListRequest, ListResponse<InstansiResponse>>
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork<ApplicationDBContext> _context;
        public GetInstansiListHandler(
            ILogger<GetInstansiListHandler> logger,
            IMapper mapper,
        ...
        public async Task<ListResponse<InstansiResponse>> Handle(GetInstansiListRequest request, CancellationToken cancellationToken)
        {
            ListResponse<InstansiResponse> result = new ListResponse<InstansiResponse>();
            try
            {
				var query = _context.Entity<Vleko.SiPeneliti.Data.Model.Instansi>().AsQueryable();

				#region Filter
				Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, object>> column_sort = null;
				List<Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, bool>>> where = new List<Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, bool>>>();
				...
				result.List = _mapper.Map<List<InstansiResponse>>(data_list);
            ...
            {
                _logger.LogError(ex, "Failed Get List Instansi", request);
                result.Error("Failed Get List Instansi", ex.Message);
            }
            return result;
        }

        #region List Utility
		private (Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, bool>> where, Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, object>> result_order = null;
			Expression<Func<Vleko.SiPeneliti.Data.Model.Instansi, bool>> result_where = null;
