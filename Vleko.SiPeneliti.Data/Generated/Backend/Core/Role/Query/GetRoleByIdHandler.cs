...
namespace Vleko.SiPeneliti.Core.Role.Query
{
    ...
    internal class GetRoleByIdHandler : IRequestHandler<GetRoleByIdRequest, ObjectResponse<RoleResponse>>
    {
        ...
        public async Task<ObjectResponse<RoleResponse>> Handle(GetRoleByIdRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var item = await _context.Entity<Vleko.SiPeneliti.Data.Model.Role>().Where(d => d.Id == request.Id).FirstOrDefaultAsync();
                if (item != null)
                {
                    ...
                    result.NotFound($"Id Vleko.SiPeneliti.Data.Model.Role {request.Id} Tidak Ditemukan");
            }
            catch (Exception ex)
            {
