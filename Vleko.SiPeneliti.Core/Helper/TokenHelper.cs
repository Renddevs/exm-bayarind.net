...
namespace Vleko.Bayarind.Core.Helper
{
    public interface ITokenHelper
    {
        ObjectResponse<TokenObject> GenerateToken(TokenUserObject request);
    ...
    public class TokenHelper : ITokenHelper
    {
        ...
        public ObjectResponse<TokenObject> GenerateToken(TokenUserObject request)
        {
            ...
            try
            {
                ...
                var claims = new List<Claim> {
                                ...
                                new Claim("role_id", request.Role.Id ?? ""),
                                new Claim("role_name", request.Role.Nama ?? ""),
                                new Claim("token" , refresh_token)
                            };


                var key = Encoding.ASCII.GetBytes(_config.SecretKey);
                DateTime expired = DateTime.UtcNow.AddMinutes(_config.TokenExpired);
            ...
            try
            {
                ...
                if (identity.Claims != null && identity.Claims.Count() > 0)
                {
                    ...
                    result.Data.User = new TokenUserObject()
                    {
                        ...
                        Role = new ReferensiStringObject()
                        {
                            Id = identity.Claims.FirstOrDefault(x => x.Type == "role_id")?.Value,
                            Nama= identity.Claims.FirstOrDefault(x => x.Type == "role_name")?.Value
                        }
                    };
                    result.OK();
                }
